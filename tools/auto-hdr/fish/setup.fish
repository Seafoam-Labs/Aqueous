...
set -l bootstrap_python python3
...
if not test -x $venv/bin/python
    ...
    if not command -q $bootstrap_python
        echo "Python 3.12 or 3.14 is required for the configured ROCm wheels." >&2
        echo "Install Python or set AUTO_HDR_BOOTSTRAP_PYTHON to its path." >&2
        exit 1
    end
...
$venv/bin/python -c 'import sys; raise SystemExit(0 if sys.version_info[:2] in ((3, 12), (3, 14)) else 1)'
or begin
    echo "ROCm venv must use Python 3.12 or 3.14: $venv" >&2
