...
/// <summary>
/// Stage 5 unit tests for <see cref="ManagerRequestSender"/>. The class
/// owns the small set of helpers that marshal Wayland requests to
/// <c>river_window_manager_v1</c> plus the manage-cycle flush flag.
///
/// <para>
/// The Wayland P/Invokes themselves cannot run in a unit-test process
/// (libwayland-client requires a live display connection). We assert
/// the observable contract: <c>InsideManageSequence</c> round-trips,
/// <c>SendManagerRequest</c> + <c>ScheduleManage</c> no-op when the
/// manager isn't bound, <c>ScheduleManage</c> short-circuits inside a
/// manage sequence, and the <c>IsBound</c> probe tracks <c>Init</c>.
/// </para>
/// </summary>
public sealed class ManagerRequestSenderTests
{
    ...
    {
        var s = new ManagerRequestSender();
        // No Init() — calling Send must be a silent no-op (no P/Invoke
        // dispatched against IntPtr.Zero, which would crash the host).
    ...
    {
        // We cannot bind in a unit test (no live wl_display), but we can
        // assert the short-circuit logic by ensuring InsideManageSequence
        // never trips a P/Invoke through the unbound guard path either.
