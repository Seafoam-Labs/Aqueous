...
public sealed class ScrollingLayout : ILayoutEngine
{
        ...
        ref object? perOutputState)
    {
        ...
        {
            state.FocusedIdx = state.Columns.Count - 1;
        }
        ...
        ref object? perOutputState)
    {
        ...
        if (idx < 0)
        {
            idx = state.FocusedIdx;
        }

        if (idx < 0 || idx >= state.Columns.Count)
        {
            return null;
        }
        ...
        int next = idx + step;
        if (next < 0 || next >= state.Columns.Count)
        {
            return null;
        }
    ...
    {
        var state = perOutputState as ScrollState;
        if (state == null || state.Columns.Count < 2)
        {
            return false;
        }
        ...
        int target = idx + step;
        if (target < 0 || target >= state.Columns.Count)
        {
            return false;
        }
        ...
        ref object? perOutputState)
    {
        ...
        if (next >= state.Columns.Count)
        {
            next = state.Columns.Count - 1;
        }
