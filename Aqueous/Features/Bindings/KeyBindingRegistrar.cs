...
/// <summary>
/// Lift of the <c>RiverWindowManagerClient.KeyBindingRegistrar.cs</c> partial into a top-level
/// service. Owns the per-seat chord declaration loop (<see cref="RegisterAllBindings"/>), the two
/// per-binding declare helpers (built-in + custom), and the per-key event dispatcher (<see
/// cref="HandleKeyBindingEvent"/>). The class <c>OnKeyBindingEvent</c> /
/// <c>RegisterAllBindings</c> / dedupe sets are gone; the god class now delegates its two
/// pass-through accessors (<c>RegisterAllBindingsForwarding</c>,
/// <c>IsBindingRegisteredForwarding</c>) and the <c>HandleKeyBindingEvent</c> bridge straight to
/// this class.
/// <para>
/// Construction still takes a <see cref="RiverWindowManagerClient"/> reference because the
/// registrar reads/writes class state that has not yet been lifted into its own singletons:
/// <c>_xkbBindings</c> + version, the per-binding <c>_keyBindings</c> dict (also used by the
/// diagnostic forwarder <c>IsBindingRegisteredForwarding</c>), the <c>_customBindingActions</c>
/// dict, the GCHandle that backs the native dispatcher's user-data pointer, the active <see
/// cref="LayoutConfig"/>, and the two binding-trio routers. Each of those is reached through a
/// small set of <c>internal</c> accessors on the god class (<c>XkbBindings</c>,
...
{
    // Dedupe sets keyed by (seat, keysym, modifiers, action) so the same (seat, chord, action) triple
    // isn't registered twice when SeatInformation fires more than once or a second seat appears.
    ...
    // Action_name -> KeyBindingAction (for built-in chord overrides via [keybinds]). Mirrors the
    // prior RiverWindowManagerClient.BuiltinActionMap alias; kept as a thin re-export of the
    // top-level table so any external references through this class still compile.
    internal static readonly IReadOnlyDictionary<string, KeyBindingAction> BuiltinActionMap =
        KeyBindingActionTable.Map;

    // Cut off RiverWindowManagerClient. All class accessors. RiverLog replaces the class static Log
    // forwarder.
    ...
    /// <summary>
    /// Register every keybind defined by the active <c>LayoutConfig.Keybinds</c> (built-in actions
    /// with config-overridable chords + custom chords with free-form action verbs). Falls back to the
    /// default chord table for any built-in not explicitly listed in the config.
    /// </summary>
    ...
    private void RegisterKeyBinding(IntPtr seatProxy, uint keysym, uint modifiers, KeyBindingAction action)
    {
        ...
        // River_xkb_bindings_v1::get_xkb_binding opcode=1 args: seat(o), id(new_id), keysym(u),
        // modifiers(u) Bind the child proxy at the parent's advertised version (not a hardcoded literal),
    ...
    /// <summary>
    /// Per-key event entry point. Invoked by the class <c>HandleKeyBindingEvent</c> bridge (which is
    /// in turn called by <c>KeyBindingEventHandler</c>). Behaviour byte-for-byte equivalent to the
    /// prior class <c>OnKeyBindingEvent</c>.
    /// </summary>
