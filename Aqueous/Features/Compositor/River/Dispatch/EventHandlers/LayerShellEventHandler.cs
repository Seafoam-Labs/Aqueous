...
/// <summary>
/// First <see cref="IEventHandler"/> extracted out of the <c>RiverWindowManagerClient</c> god
/// class. Handles the single <c>river_layer_shell_v1::layer_surface</c> event (opcode 0): on each
/// newly-mapped layer surface, ask the compositor for its <c>river_node_v1</c> child and place it
/// above existing nodes. No managed state, no service dependencies — the body is a pair of
/// <c>wl_proxy_marshal_flags</c> P/Invokes plus a log line. The optional <see
/// cref="Action{String}"/> log sink keeps the handler trivially unit-testable without pulling in
/// <c>Microsoft.Extensions.Logging</c>. confirmed zero class coupling — the ctor takes only an
/// optional log sink, and the body has no <c>RiverWindowManagerClient</c> reference. No cutover
/// required for this handler; Pump-thread only: invoked by the native callback in
