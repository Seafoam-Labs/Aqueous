...
/// <summary>
/// Client-side adapter for the <c>wlr-screencopy-unstable-v1</c> protocol that Aqueous now
...
/// same one running <see cref="RiverWindowManagerClient"/>'s pump). The <see
/// cref="CaptureOutputAsync"/> flow registers a frame proxy, then resolves the <see
/// cref="Task{TResult}"/> from inside <see cref="OnFrameEvent"/> when the <c>ready</c> or
/// <c>failed</c> event arrives.
/// </para>
/// <para>
/// Buffer backing uses the <c>wl_shm</c> path with a <c>memfd_create</c> fd — the simplest,
/// AOT-friendly option that requires no extra runtime dependencies. A DMA-BUF path can be added
/// later behind the same public surface; that requires <c>linux-dmabuf-unstable-v1</c> and
/// <c>libgbm</c>.
/// </para>
/// </summary>
internal sealed unsafe partial class WlrScreencopyClient : IDisposable
{
    ...
    /// <summary>
    /// In-flight captures keyed by their <c>zwlr_screencopy_frame_v1</c> proxy handle.
    /// </summary>
    private readonly ConcurrentDictionary<IntPtr, FrameCapture> _frames = new();

    /// <summary>
    /// Constructs the screencopy client. Both <paramref name="manager"/> and <paramref name="shm"/>
    /// must already have been bound from the registry; the constructor does not bind anything itself.
    /// </summary>
    /// <param name="manager">
    /// A bound <c>zwlr_screencopy_manager_v1</c> proxy.
    /// </param>
    /// <param name="version">
    /// Negotiated manager version (1.3).
    /// </param>
    /// <param name="shm">
    /// A bound <c>wl_shm</c> proxy.
    /// </param>
    /// <param name="selfHandle">
    /// GCHandle (as IntPtr) routed to the dispatcher's implementation slot.
    /// </param>
    /// <param name="dispatcher">
    /// Function pointer for <see cref="WaylandInterop.wl_proxy_add_dispatcher"/>.
    /// </param>
    ...
    /// <summary>
    /// Begins a full-output capture. Returns a task that completes with a freshly allocated RGBA-ish
    /// byte buffer (actual pixel format reported in <see cref="ScreencopyResult.Format"/>) once the
    /// compositor signals <c>ready</c>, or faults with <see cref="IOException"/> on <c>failed</c>.
    /// </summary>
    /// <param name="output">
    /// A <c>wl_output</c> proxy that the client already owns.
    /// </param>
    /// <param name="overlayCursor">
    /// When <c>true</c>, the cursor is composited into the captured frame.
    /// </param>
    /// <returns>
    /// Awaitable result describing the captured frame.
    /// </returns>
    /// <remarks>
    /// Must be invoked on the dispatch thread (or with a queue serialised against it). The caller is
    /// responsible for not destroying <paramref name="output"/> while the capture is in flight.
    /// </remarks>
    ...
    /// <summary>
    /// Routes a single libwayland event for an in-flight <c>zwlr_screencopy_frame_v1</c> proxy.
    /// </summary>
    /// <param name="frame">
    /// The frame proxy receiving the event.
    /// </param>
    /// <param name="opcode">
    /// Event opcode (see <see cref="RiverProtocolOpcodes.ScreencopyFrame"/>).
    /// </param>
    /// <param name="args">
    /// Pointer to the libwayland argument array.
    /// </param>
    /// <returns>
    /// <c>true</c> when the proxy is known to this client and the event was consumed.
    /// </returns>
    /// <remarks>
    /// Called from the central <see cref="RiverWindowManagerClient"/> dispatcher; never throws into
    /// native code.
    /// </remarks>
    public bool OnFrameEvent(IntPtr frame, uint opcode, WlArgument* args)
    {
        ...
        switch (opcode)
        {
            case RiverProtocolOpcodes.ScreencopyFrame.Buffer:
                ...
                // For protocol v1/v2 there's no buffer_done, so allocate + copy as soon as we see the buffer
                // event. v3 waits for buffer_done so it can also process linux_dmabuf.
                if (_version < 3)
                {
    ...
    /// <summary>
    /// Per-capture state held between <c>capture_output</c> and <c>ready</c>.
    /// </summary>
...
/// </summary>
/// <param name="Pixels">
/// Raw pixel bytes (length = <paramref name="Stride"/> × <paramref name="Height"/>).
/// </param>
/// <param name="Width">
...
/// Row stride in bytes.
/// </param>
/// <param name="ShmFormat">
/// The <c>wl_shm</c> format enum reported by the compositor (e.g. XRGB8888 = 1).
/// </param>
/// <param name="Flags">
/// <c>zwlr_screencopy_frame_v1.flags</c> bitset. Currently the only defined bit is <c>Y_INVERT</c>
/// (1) — when set, the rows are bottom-up.
/// </param>
