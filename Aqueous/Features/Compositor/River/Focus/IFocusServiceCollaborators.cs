...
///
/// <para>
/// As of Stage 5 the following members were retired and replaced by
/// injected services: <c>ScheduleManage</c> (-> <c>IManagerRequestSender</c>),
/// <c>ResolveOutputName</c> / <c>BuildSnapshotFor</c> / <c>LayoutFocusNeighbor</c>
/// (-> <c>ILayoutProposer</c>). The remaining members all require
/// god-class private-state migration and are retired in Stages 5b / 9.
/// </para>
///
/// <para>
...
internal interface IFocusServiceCollaborators
{
    ...
    /// <summary>Updates the pending-focus fields used by the manage_start flush. -> retired in Stage 5b (god-class field migration).</summary>
    void SetPendingFocusWindow(IntPtr windowProxy, IntPtr seatProxy);
    /// <summary>Updates the pending-focus fields for a layer-shell surface. -> retired in Stage 5b.</summary>
    void SetPendingFocusShellSurface(IntPtr shellSurfaceProxy, IntPtr seatProxy);
    /// <summary>Reads the pending-focus-window field (used by the same-focus short-circuit). -> retired in Stage 5b.</summary>
    IntPtr PendingFocusWindow { get; }
    /// <summary>Reads the pending-focus-shell-surface field. -> retired in Stage 5b.</summary>
    IntPtr PendingFocusShellSurface { get; }
    /// <summary>Send river_seat_v1::clear_focus (opcode 3) on the given seat. -> retired in Stage 5b.</summary>
