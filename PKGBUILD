...
depends=('ghostty' 'sudo' 'shelly' 'python' 'gtk4' 'freetype2' 'wayland' 'wayland-protocols>=1.49' 'libxkbcommon' 'libinput'
         'pixman' 'libpng' 'libdrm' 'libevdev'
         'libdecor' 'grim' 'slurp' 'xorg-xwayland'
...
package() {
    AQUEOUS_CONFIG_BINARY="$srcdir/aqueous-config-dist/bin/aqueous-config" \
    AQUEOUSCTL_BINARY="$srcdir/aqueous-dist/bin/aqueousctl" \
        DESTDIR="$pkgdir" PREFIX=/usr "$srcdir/aqueous/settingsApplication/packaging/install.sh" --with-dms-appearance
    ...
    # Shell startup units are installed by install-welcome.sh below.
    ...
    AQUEOUS_WELCOME_BINARY="$srcdir/aqueous-welcome-dist/bin/aqueous-welcome" \
        DESTDIR="$pkgdir" sh "$srcdir/aqueous/packaging/install-welcome.sh"
}
