...
helper=${1:-"$plugin_root/zig-out/bin/aqueous-backend-test"}
...
if command -v gsettings >/dev/null 2>&1; then
    ...
    for schema in org.gnome.desktop.interface.gschema.xml org.gnome.desktop.enums.xml; do
        if [[ ! -r "$schema_source/$schema" ]]; then
            echo "Backend tests require $schema_source/$schema; install gsettings-desktop-schemas in the build root" >&2
            exit 1
        fi
...
echo "Embedded configuration integration tests passed"

python3 "$plugin_root/tests/backend/test-modes.py" "$helper"
