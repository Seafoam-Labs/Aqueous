...
pub const Shell = enum { noctalia, dms, none };
...
pub fn inspectForShell(allocator: Allocator, io: std.Io, spec: *const FontSpec, shell: Shell) Report {
    ...
    return .{ .targets = .{
        if (shell != .noctalia) status("noctalia", false, false, false) else statusWithCapability("noctalia", true, true, documentMatches3(allocator, noctalia_path, .{
...
pub fn applyForShell(allocator: Allocator, io: std.Io, spec: *const FontSpec, shell: Shell) Report {
    ...
    return .{ .targets = .{
        if (shell != .noctalia) status("noctalia", false, false, false) else appliedStatusWithCapability("noctalia", true, true, noctalia_ok, spec.isSpecific()),
