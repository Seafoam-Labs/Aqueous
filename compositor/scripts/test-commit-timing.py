#!/usr/bin/env python3
"""Commit timing wire/cache tests and numbered-frame tests in a private Aqueous session.
...
def main():
    ...
    parser.add_argument('--policy', choices=('internal', 'external', 'compare'), default='internal', help='Non-internal modes run registry/lifetime smoke checks')
    parser.add_argument('--unit-only', action='store_true')
    args = parser.parse_args()
    work = Path(tempfile.mkdtemp(prefix='aqueous-commit-timing-tests-'))
    ...
    def burst(client, label, count=8):
        ...
        assert all(e['presented_ns'] >= e['requested_ns'] > 0 for e in selected), selected
        return selected
    ...
    try:
        ...
        for name, xml in {
            ...
            'commit-timing-v1': '/usr/share/wayland-protocols/staging/commit-timing/commit-timing-v1.xml',
        ...
        for case in ('queue', 'timer', 'many', 'subsurface', 'waits', 'lifetime', 'reentry', 'parent-destroy', 'duplicate', 'invalid', 'timestamp-exists', 'recreate-pending', 'surface-destroyed', 'bypass'):
            ...
            expected_assertion = 's->count == 0'
            if negative:
                assert result.returncode == -signal.SIGABRT and expected_assertion in result.stdout, result.stdout
            else:
                assert result.returncode == 0, result.stdout
        ...
        checked(['cc', '-std=c11', '-Wall', '-Wextra', '-Werror', '-O2', f'-I{work}',
                 ROOT / 'scripts/fixtures/commit-timing-v1-client.c', *generated, '-lwayland-client', '-o', work / 'client'])
        security_xml = Path(checked(['pkg-config', '--variable=pkgdatadir', 'wayland-protocols']).strip()) / 'staging/security-context/security-context-v1.xml'
        checked(['wayland-scanner', 'client-header', security_xml, work / 'security-context-client-protocol.h'])
        checked(['wayland-scanner', 'private-code', security_xml, work / 'security-context.c'])
        checked(['cc', '-std=c11', '-Wall', '-Wextra', '-Werror', '-O2', f'-I{work}',
                 ROOT / 'scripts/fixtures/commit-timing-policy.c', generated[1], work / 'security-context.c',
                 '-lwayland-client', '-o', work / 'policy'])
        for key in list(env):
...
''')
        ...
        compositor = launch([args.compositor.resolve(), '-no-xwayland', '-policy', args.policy, '-log-level', 'info', '-c', 'true'], 'compositor')
        env['WAYLAND_DISPLAY'] = wait(lambda: next((p.name for p in runtime.glob('wayland-*') if p.is_socket()), None), 'no Wayland socket')
        checked([work / 'policy', 'sandbox'])
        record(f'{args.policy}-registry-lifetime-and-security-context')
        ...
        client.stdin.write(b'x'); client.stdin.flush()
        wait(lambda: all(any(e['event'] == 'presented' and e['frame'] == frame for e in events()) for frame in (previous + 1, previous + 2)), 'destroyed timer lost queued constraints')
        record('destroy-timer-then-untimed-commit')
        previous = max(e['frame'] for e in events() if e['event'] == 'submitted')
        client.stdin.write(b'n'); client.stdin.flush()
        wait(lambda: any(e['event'] == 'presented' and e['frame'] == previous + 1 for e in events()), 'time-only update did not wake output')
        record('idle-wakeup-without-fifo')
        ...
        for i in range(3):
            burst(client, 'client')
        record('sustained-queue', frames=24)
        time.sleep(.2)
        ...
        request(op='set', changes=[dict(name=n, enabled=False) for n in names])
        previous = max(e['frame'] for e in events() if e['event'] == 'submitted')
        client.stdin.write(b'n'); client.stdin.flush()
        wait(lambda: any(e['event'] == 'submitted' and e['frame'] == previous + 1 for e in events()), 'output-less update was not submitted')
        time.sleep(.3)
        request(op='set', changes=[dict(name=n, enabled=True) for n in names])
        wait(lambda: any(e['event'] == 'presented' and e['frame'] >= previous + 1 for e in events()), 'output-less timed update did not resume')
        record('all-outputs-disabled-deadline-and-resume')
        timing_samples = [e for label in ('client', 'fast', 'cover') for e in events(label)
                          if e['event'] == 'presented' and e['requested_ns']]
        late_ms = sorted((e['presented_ns'] - e['requested_ns']) / 1e6 for e in timing_samples)
        assert late_ms and late_ms[0] >= 0
        record('presentation-deadlines', samples=len(late_ms), minimum_ms=late_ms[0],
               median_ms=late_ms[len(late_ms) // 2], p95_ms=late_ms[int((len(late_ms) - 1) * .95)],
               maximum_ms=late_ms[-1])
    finally:
