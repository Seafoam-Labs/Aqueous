...
static bool attach_auxiliary_buffer(
    struct app *app,
    struct wl_surface *surface,
    int32_t width,
    int32_t height,
...
static void publish_ready_when_complete(struct app *app) {
    if (app->configured) return;
    if (app->role == ROLE_BLUR && !app->popup_configured) return;
    app->configured = true;
...
static void popup_surface_configure(
    ...
    if (!app->popup_configured) {
        if (!attach_auxiliary_buffer(
                app,
                app->popup_surface,
                220,
                140,
                argb(255, 240, 48, 180),
                argb(255, 52, 224, 245))) {
            fail(app, "unable to allocate the popup buffer");
            return;
        }
...
static void popup_done(void *data, struct xdg_popup *popup) {
    (void)popup;
    fail(data, "compositor dismissed the reference popup");
}
...
static bool create_auxiliary_surfaces(struct app *app) {
    if (app->subcompositor == NULL) return false;

    app->subsurface_surface = wl_compositor_create_surface(app->compositor);
    if (app->subsurface_surface == NULL) return false;
    app->subsurface = wl_subcompositor_get_subsurface(
        app->subcompositor,
        app->subsurface_surface,
        app->surface);
    if (app->subsurface == NULL) return false;
    wl_subsurface_set_position(app->subsurface, 72, 84);
    wl_subsurface_set_desync(app->subsurface);
    if (!attach_auxiliary_buffer(
            app,
            app->subsurface_surface,
            180,
    ...
    xdg_positioner_set_anchor_rect(positioner, 320, 180, 1, 1);
...
static void surface_configure(
    ...
    if (app->role == ROLE_BLUR && app->popup == NULL) {
        if (!create_auxiliary_surfaces(app)) {
            fail(app, "unable to create popup and subsurface references");
            return;
        }
    }
    publish_ready_when_complete(app);
...
int main(int argc, char **argv) {
    ...
    if (wl_display_roundtrip(app.display) < 0 || app.compositor == NULL ||
        app.subcompositor == NULL || app.shm == NULL || app.wm_base == NULL) {
    ...
    if (app.popup_xdg_surface != NULL) xdg_surface_destroy(app.popup_xdg_surface);
    if (app.popup_surface != NULL) wl_surface_destroy(app.popup_surface);
    if (app.subsurface != NULL) wl_subsurface_destroy(app.subsurface);
    if (app.subsurface_surface != NULL) wl_surface_destroy(app.subsurface_surface);
