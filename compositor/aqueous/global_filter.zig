...
pub fn dynamicallyBlocklistedInterface(interface_name: [*:0]const u8) bool {
    return std.mem.orderZ(u8, interface_name, "wp_drm_lease_device_v1") == .eq or
        std.mem.orderZ(u8, interface_name, "aqueous_output_warming_manager_v1") == .eq;
}
...
test "every DRM lease device is privileged without blocklisting wl_output" {
    ...
    try std.testing.expect(dynamicallyBlocklistedInterface("aqueous_output_warming_manager_v1"));
